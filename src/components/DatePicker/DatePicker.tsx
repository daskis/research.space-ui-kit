...
import { CalendarType, IDatePickerProps } from './DatePicker.props.ts';
import dayjs, { Dayjs } from 'dayjs';
import { classNames } from '@helpers';
import { Paragraph } from '@components';
import { Years } from './Years/index.ts';
import { Months } from './Months/index.ts';
import { Days } from './Days/index.ts';

export const DatePicker = ({ size = 'medium', color = 'primary', onEnd, borderRadius = 10 }: IDatePickerProps) => {
            ...
            <div className={cls.heading}>
                ...
                <div className={cls.date}>
                    <button onClick={() => setCurrentTab(CalendarType.YEARS)} className={cls.btn}>
                        <Paragraph size={textSize}>{selectedYear}</Paragraph>
                    </button>
                    <button onClick={() => setCurrentTab(CalendarType.MONTHS)} className={cls.btn}>
                        <Paragraph size={textSize}>{dayjs().month(selectedMonth).format('MMMM').slice(0, 3)}</Paragraph>
                    </button>
                </div>
                <div className={cls.buttonWrapper}>
                    <button
